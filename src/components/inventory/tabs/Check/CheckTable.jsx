...
const CheckTable = ({
  ...
  const toggleRowExpand = async (docId) => {
    ...
    try {
      const res = await getCheckById(docId);
      setLoadedDoc(res || {});
    } catch (err) {
      console.error('Failed to load check document details:', err);
      setLoadedDoc(null);
    } finally {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', width: 280 }}>
            ...
            <input
              type="text"
              placeholder="Theo mã phiếu kiểm kho, người tạo..."
              value={searchText}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
            ...
            onClick={() => {
              ...
              else if (setModalCheckOpen) setModalCheckOpen(true);
            }}
            style={{
      ...
      <div style={{ flex: 1, overflow: 'auto', border: '1px solid #cbd5e1', borderRadius: 6 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, textWrap: 'nowrap' }}>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', padding: 20, color: '#64748b' }}>Đang tải dữ liệu...</td>
              </tr>
            ) : currentData.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', padding: 20, color: '#94a3b8' }}>Không tìm thấy phiếu kiểm kho nào</td>
              </tr>
            ) : (
              currentData.map((doc) => {
                ...
                return (
                  <React.Fragment key={doc.id}>
                    ...
                    >
                      <td style={{ padding: '8px 10px', fontWeight: 700, color: '#d97706' }}>{doc.code}</td>
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (window.confirm(`Bạn có chắc chắn muốn ${isPending ? 'hủy phiếu nháp' : 'xóa chứng từ'} này?`)) {
                                handleDeleteDocument(doc.id, doc.type || 10, doc.status);
                              }
                            }}
                      ...
                      <td style={{ padding: '8px 10px', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          onClick={() => navigate(`/check-detail/${doc.id}`)}
                          style={{
                            padding: '3px 10px',
                            fontSize: 11,
                            borderRadius: 4,
                        ...
                        <td colSpan={8} style={{ padding: '0 12px 14px 12px', borderBottom: '2px solid #cbd5e1' }}>
                          ...
                          >
                            ...
                            <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', background: '#f8fafc', padding: '0 12px', alignItems: 'center', justifyContent: 'space-between' }}>
                              ...
                              >
                                <InfoCircleOutlined /> Thông tin chi tiết phiếu kiểm kho
                              </div>
                              <button
                                type="button"
                                onClick={() => navigate(`/check-detail/${doc.id}`)}
                            ...
                            {loadingDocId === doc.id ? (
                              <div style={{ padding: 24, textAlign: 'center', color: '#64748b' }}>
                                Đang nạp chi tiết phiếu kiểm kho...
                              </div>
                            ) : (
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                                  <div>
                                    ...
                                    <div style={{ color: '#64748b', fontSize: 11, marginTop: 4 }}>
                                      Người tạo: <strong style={{ color: '#334155' }}>{activeDoc.createdByName || activeDoc.snapshotCreatedByName || activeDoc.currentCreatedByName || activeDoc.creatorName || activeDoc.creator || doc.createdByName || doc.creatorName || 'Hệ thống'}</strong> &nbsp;|&nbsp; Thời gian: <strong style={{ color: '#334155' }}>{dayjs(activeDoc.orderDate || activeDoc.businessDate || activeDoc.createdAt || doc.orderDate || doc.createdAt).format('DD/MM/YYYY HH:mm')}</strong>
                                    </div>
                                  </div>
                                  ...
                                  <div style={{ color: '#475569', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                                    ...
                                    <span>{activeDoc.branchName || activeDoc.snapshotBranchName || activeDoc.currentBranchName || doc.branchName || 'Chi nhánh trung tâm'}</span>
                                  </div>
                                </div>
                                  ...
                                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                                    <thead>
                                      <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569', fontWeight: 600, textAlign: 'left' }}>
                                        ...
                                        <th style={{ padding: '8px 10px', textAlign: 'right' }}>Tồn hệ thống</th>
                                        <th style={{ padding: '8px 10px', textAlign: 'right' }}>Thực tế kiểm</th>
                                        <th style={{ padding: '8px 10px', textAlign: 'right' }}>Chênh lệch SL</th>
                                        <th style={{ padding: '8px 10px', textAlign: 'right' }}>Giá vốn (đ)</th>
                                        <th style={{ padding: '8px 10px', textAlign: 'right' }}>Giá trị lệch (đ)</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      {detailsList.length === 0 ? (
                                        ...
                                        detailsList.map((dt, idx) => {
                                          ...
                                          const conversionRate = Number(dt.conversionRate || dt.currentConversionRate || 1);
                                          const uName = dt.snapshotUnitName || dt.currentUnitName || dt.unitName || dt.snapshotBaseUnitName || dt.currentBaseUnitName || 'Đơn vị';

                                          const sysStock = Number(dt.systemQuantity ?? dt.currentStockQuantity ?? dt.currentStock ?? 0) / conversionRate;
                                          const actualQty = dt.quantity != null ? Number(dt.quantity) : (dt.actualQuantity != null ? Number(dt.actualQuantity) / conversionRate : 0);
                                          const diffQty = actualQty - sysStock;
                                          const costPrice = Number(dt.snapshotAvgCost || dt.unitPrice || 0) * conversionRate;
                                          ...
                                          return (
                                            <tr key={dt.id || idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                              ...
                                              <td style={{ padding: '8px 10px', textAlign: 'right', color: '#475569' }}>{sysStock.toLocaleString('vi-VN', { maximumFractionDigits: 3 })}</td>
                                              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, color: '#0f172a' }}>{actualQty.toLocaleString('vi-VN', { maximumFractionDigits: 3 })}</td>
                                              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, color: diffQty > 0 ? '#059669' : (diffQty < 0 ? '#dc2626' : '#64748b') }}>
                                                {diffQty > 0 ? `+${diffQty.toLocaleString('vi-VN', { maximumFractionDigits: 3 })}` : diffQty.toLocaleString('vi-VN', { maximumFractionDigits: 3 })}
                                              </td>
                                              <td style={{ padding: '8px 10px', textAlign: 'right', color: '#475569' }}>{Math.round(costPrice).toLocaleString('vi-VN')} đ</td>
                                              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, color: diffAmount > 0 ? '#059669' : (diffAmount < 0 ? '#dc2626' : '#64748b') }}>
                                                {diffAmount > 0 ? `+${Math.round(diffAmount).toLocaleString('vi-VN')}` : Math.round(diffAmount).toLocaleString('vi-VN')} đ
                                              </td>
                                            </tr>
                                          );
                                        })
                                ...
                                {isPending && (
                                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 14, paddingTop: 12, borderTop: '1px border-dashed #cbd5e1' }}>
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => {
                                        if (window.confirm('Bạn có chắc chắn muốn xóa phiếu kiểm kho nháp này?')) {
                                          handleDeleteDocument(doc.id, doc.type || 10, doc.status);
                                        }
                                      }}
                                      style={{
