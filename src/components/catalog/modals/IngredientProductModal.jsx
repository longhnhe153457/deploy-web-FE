...
import { createIngredientProduct, updateIngredientProduct } from '../../../api/productApi';
import { uploadImage } from '../../../api/imageApi';
import UnitConversionBuilder from './UnitConversionBuilder';

const IngredientProductModal = ({
  open,
  onCancel,
  editingProduct = null,
  groups = [],
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      notifyError('Vui lòng nhập tên nguyên liệu.');
      return;
    }
    if (!formData.groupId) {
    ...
    try {
      ...
      if (editingProduct && editingProduct.id) {
        ...
        await updateIngredientProduct(payload);
        notifySuccess('Cập nhật nguyên liệu thành công!');
      } else {
        await createIngredientProduct(payload);
        notifySuccess('Tạo mới nguyên liệu thành công!');
      }
      onSuccess();
      onCancel();
    } catch (err) {
      const msg = err.response?.data?.message || 'Có lỗi xảy ra khi lưu nguyên liệu.';
      notifyError(msg);
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: '#7c2d12' }}>
                {editingProduct ? 'Cập Nhật Nguyên Vật Liệu' : 'Thêm Nguyên Vật Liệu Mới'}
              </h3>
              <span style={{ fontSize: 12, color: '#c2410c' }}>INGREDIENT - Nguyên liệu kho/bếp (Không có giá bán & menu)</span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ padding: 24, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 20 }}>
              ...
              <div>
                ...
                >
                  {uploadingImage ? (
                    <div style={{ textAlign: 'center', color: '#ea580c', fontSize: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={{ gridColumn: 'span 2' }}>
                  ...
                  <input
                    type="text"
                    placeholder="VD: Sữa tươi Vinamilk, Hạt cà phê Robusta, Bột mì"
                    value={formData.name}
            ...
            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#334155', marginBottom: 4 }}>Mô tả nguyên liệu</label>
              <textarea
                rows={3}
          ...
          <div style={{ padding: '16px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: 12, background: '#f8fafc' }}>
            ...
            <button type="submit" disabled={loading} style={{ padding: '9px 24px', borderRadius: 8, border: 'none', background: '#ea580c', color: '#ffffff', cursor: 'pointer', fontWeight: 600 }}>
              {loading ? 'Đang lưu...' : editingProduct ? 'Cập Nhật' : 'Tạo Mới nguyên liệu'}
            </button>
          </div>
        </form>
      </div>
...
export default IngredientProductModal;
